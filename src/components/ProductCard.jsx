  ...
  return (
    <article className="card card-clickable" onClick={onClick}>
      <div style={{ height:180, background:'#1a1a1a', position:'relative', overflow:'hidden' }}>
        ...
        {stockStatus !== 'ok' && (
          <div style={{ position:'absolute', top:8, right:8, background:stockStatus==='out'?'var(--danger)':'var(--warning)', color:'#000', fontSize:10, fontWeight:700, borderRadius:20, padding:'3px 8px' }}>
            {stockStatus === 'out' ? 'AGOTADO' : 'POCAS UNIDADES'}
          </div>
        )}
      </div>
