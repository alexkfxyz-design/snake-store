  ...
  return (
    <div style={{ minHeight:'100vh' }}>
      ...
      <div className="catalog-hero">
        ...
        <p className="catalog-hero-sub">Ropa y accesorios — {products.length} productos</p>
      </div>
      <div className="catalog-filters">
        <button className={`filter-chip ${activeCat==='all'?'active':''}`} onClick={() => setActiveCat('all')}>Todo ({products.length})</button>
        {categories.map(cat => {
          const count = products.filter(p => p.categoryId === cat.id).length
          return <button key={cat.id} className={`filter-chip ${activeCat===cat.id?'active':''}`} onClick={() => setActiveCat(cat.id)}>{cat.icon} {cat.name} ({count})</button>
        })}
      </div>
                ...
                return (
                  <article key={p.id} className="cat-product-card">
                    <div className="cat-product-img">
                      {p.image ? <img src={p.image} alt={p.name} /> : <div style={{ width:'100%', height:'100%', display:'flex', alignItems:'center', justifyContent:'center', fontSize:56, opacity:.2 }}>{cat?.icon||'📦'}</div>}
                      {ss==='out' && <div style={{ position:'absolute', inset:0, background:'rgba(0,0,0,.6)', display:'flex', alignItems:'center', justifyContent:'center' }}><span style={{ background:'var(--danger)', color:'#fff', fontWeight:700, fontSize:13, borderRadius:20, padding:'6px 16px' }}>AGOTADO</span></div>}
                      {ss==='low' && <div style={{ position:'absolute', top:10, right:10, background:'var(--warning)', color:'#000', fontSize:10, fontWeight:700, borderRadius:20, padding:'3px 10px' }}>ÚLTIMAS UNIDADES</div>}
                    </div>
      ...
      <footer style={{ borderTop:'1px solid var(--border)', padding:'2rem', textAlign:'center', color:'var(--muted)', fontSize:13 }}>
        <span style={{ fontFamily:'var(--fd)', fontSize:20, color:'var(--accent)', letterSpacing:'.1em', marginRight:12 }}>SNAKE</span>Ropa y accesorios
      </footer>
    </div>
  )
}
