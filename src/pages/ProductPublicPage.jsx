// src/pages/ProductPublicPage.jsx
...
export function ProductPublicPage({ productId, onBack }) {
  ...
  useEffect(() => {
    const u1 = subscribeProducts(data => {
      const found = data.find(p => p.id === productId)
      setProduct(found || null)
      setLoading(false)
    })
    const u2 = subscribeCategories(data => {
      setCategory(prev => prev ? data.find(c => c.id === prev.id) || null : null)
    })
    return () => { u1(); u2() }
  }, [productId])
  ...
  if (!product) return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'2rem' }}>
      ...
      <h1 style={{ fontFamily:'var(--fd)', fontSize:36, color:'var(--accent)', marginBottom:8, letterSpacing:'.08em' }}>PRODUCTO NO ENCONTRADO</h1>
      ...
      <div className="pub-inner">
        ...
        <div className="pub-body">
          ...
          <div className="stock-pill" style={{ background:sc.bg, border:`1px solid ${sc.border}`, color:sc.color }}>
            {stockStatus==='out' ? '⚠ Sin stock' : stockStatus==='low' ? `⚡ Últimas ${product.stock} unidades` : `✓ ${product.stock} unidades disponibles`}
          </div>
